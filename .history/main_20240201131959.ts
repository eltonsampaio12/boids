...
import { updateBehavior, updateBoidsPerceptionRadius } from "./utils";
...
if (separationSlider) {
  separationSlider.oninput = function () {
    const value: number = parseFloat(separationSlider.value);
    if (separationValue && value) {
      separationValue.innerHTML = String(value);
      updateBehavior("SEPARATION_BEHAVIOR", String(value), boids);
    }
  };
}
