...
export class Boid {
  ...
  update(neighbors?: Boid[]): void {
    ...
    this.getVelocity().clampLength(0, 4);
  ...
  edges() {
    if (this.position.x < -window.innerWidth / 2) {
      this.acceleration.x *= -1; // Reflect the acceleration to keep the boid inside
    } else if (this.position.x > window.innerWidth / 2) {
      this.acceleration.x *= -1; // Reflect the acceleration
    }

    if (this.position.y < -window.innerHeight / 2) {
      this.acceleration.y *= -1; // Reflect the acceleration
    } else if (this.position.y > window.innerHeight / 2) {
      this.acceleration.y *= -1; // Reflect the velocity
    }
  }
